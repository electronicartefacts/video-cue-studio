...
function logicFilename() { const original = state.file?.name || state.project?.video?.filename || 'video-cue-studio'; const stem = original.replace(/\.[^.]+$/, '').replace(/[^a-z0-9._-]+/gi, '-').replace(/^-+|-+$/g, '') || 'video-cue-studio'; return `${stem}.logic-cues.wav`; }
function exportLogic() { const markers = state.markers.filter((marker) => marker.status === 'validated'); if (!markers.length) { toast('NO VALIDATED CUES — VALIDATE BEFORE EXPORT'); return; } els['export-logic'].disabled = true; els['export-logic'].textContent = 'GENERATING…'; requestAnimationFrame(() => { try { downloadBlob(logicFilename(), createLogicMarkerWav({ duration:state.duration, markers })); toast(`${markers.length} LOGIC CUE${markers.length === 1 ? '' : 'S'} EXPORTED`); } catch (error) { console.error(error); toast('LOGIC EXPORT COULD NOT BE GENERATED'); } finally { els['export-logic'].disabled = false; els['export-logic'].textContent = 'LOGIC'; } }); }
...
video.addEventListener('loadedmetadata', () => { state.duration=video.duration; video.playbackRate=state.playbackRate; clearImportProgress(); setStudio(); els['video-name'].textContent=state.file?.name || state.project?.video?.filename || 'LOCAL VIDEO'; els['video-meta'].textContent=`${video.videoWidth || '—'} × ${video.videoHeight || '—'} · ${formatTime(video.duration)}`; if ((!video.videoWidth || !video.videoHeight) && !state.compatiblePreview) { showVideoAssist('Your browser can read the audio, but cannot render this video codec. Preparing a compatible local preview now.', false); prepareForPlayback(); } else els['video-error'].hidden=true; render(); save(); });
video.addEventListener('playing', monitorVideoFrames); video.addEventListener('error', () => { if (state.compatiblePreview) { showVideoAssist('The local preview could not be rendered by this browser. Try Chrome or Safari with the original video.', false); return; } showVideoAssist('This browser cannot decode this video directly. Preparing a compatible local preview now.', false); prepareForPlayback(); }); video.addEventListener('timeupdate',()=>{ if (!state.dragging) render(); }); video.addEventListener('play',()=>els['play-button'].textContent='PAUSE'); video.addEventListener('pause',()=>els['play-button'].textContent='PLAY');
